...
/**
 * This parses Jenkins rss for LR-dev view and can say which projects are not
 * green now
 * 
...
public class LrDevTests extends DefaultHandler implements Command {
	...
	public String execute(String cmdStr) {
		...
		try {
			...
			sp.parse(RSS_LATEST_URL, new DefaultHandler() {
				...
				public void characters(char[] ch, int start, int length)
					...
					if (!(title.endsWith("(stable)") || title
							.endsWith("(back to normal)"))) {
		...
		StringBuilder sb = new StringBuilder(
				"Following projects are not green:\n");
